...
/*
	Feature B:
		Now returning the generated hex value for each iteration. The hex value is generated by the string
		generator and sent to the watcher. Then, the watcher will update the counter with the new hex value.
*/
...
func (w *Watcher) Start() error {
	...
	go func(wg *sync.WaitGroup) {
		...
		for {
			select {
			case hexValue := <-w.inCh:
				w.counterLock.Lock()
				w.counter.HexValue = hexValue
